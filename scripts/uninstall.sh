...
echo ""
echo -e "${CYAN}==========================================${NC}"
echo -e "${BOLD}  CloaudeCodeCTO Uninstaller${NC}"
echo -e "${CYAN}==========================================${NC}"
echo ""
...
if [ "$ASSUME_YES" -eq 0 ] && [ "$DRY_RUN" -eq 0 ]; then
    echo -e "${YELLOW}This will remove all components installed by CloaudeCodeCTO.${NC}"
...
if KEEP_GEN:
    ...
    if claude_md.exists():
        try:
            ...
            if "curated by CloaudeCodeCTO" in content:
    ...
    if settings.exists():
        try:
            ...
            if "Generated by CloaudeCodeCTO installer" in content:
