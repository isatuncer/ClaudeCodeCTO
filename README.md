...
Claude Code loads every installed skill's YAML frontmatter at session start. Installing 1840 skills blindly burns **~91k tokens** before you type a single prompt. CloaudeCodeCTO ships three curated profiles so you pick your own budget:
...
Installing CloaudeCodeCTO doesn't just dump files into `~/.claude/`. The installer also wires **32 hooks** across 7 event types (`PreToolUse`, `PostToolUse`, `SessionStart`, `SessionEnd`, `PreCompact`, `Stop`, `PostToolUseFailure`) so every session runs with these six capabilities active:
